...
class Candidate: Codable, ObservableObject {
    ...
    init(party: String, name: String, url: String, campaign_url: String?, thumbnail: String? = nil, policies: [Policy]? = nil) {
        self.party = party
        self.name = name
        self.url = url
    ...
    enum CodingKeys: String, CodingKey {
        case party, name, campaign_url, thumbnail, policies, url
    }
    ...
    func getPolicies() async -> Void {
        if (self.campaign_url == nil) {
            ...
            let fetchedPolicies = await DataModel.getPolicies(url: self.campaign_url!, curl: self.url)
